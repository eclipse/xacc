...
// **************************************************************
double CalculateRSS(const double *x, const double *y, const double *a,
                    double **Weights, const bool fixed, const size_t N,
                    const size_t n) {
...
// **************************************************************
double CalculateTSS(const double *x, const double *y, const double *a,
                    double **Weights, const bool fixed, const size_t N,
                    const size_t n) {
...
// **************************************************************
double CalculateR2COD(const double *x, const double *y, const double *a,
                      double **Weights, const bool fixed, const size_t N,
                      const size_t n) {
...
// **************************************************************
double CalculateR2Adj(const double *x, const double *y, const double *a,
                      double **Weights, const bool fixed, const size_t N,
                      const size_t n) {
...
void PolyFit(const double *x, double *y, const size_t n, const size_t k,
             const bool fixedinter, const double fixedinterval, double *beta,
             double **Weights, double **XTWXInv) {
...
// **************************************************************
void WriteCIBands(std::string filename, const double *x, const double *coefbeta,
                  double **XTXInv, const double tstudentval, const double SE,
...
// **************************************************************
void CalculateSERRBeta(const bool fixedinter, const double SE, size_t k,
                       double *serbeta, double **XTWXInv) {
...
// **************************************************************
void DisplayANOVA(const size_t nstar, const size_t k, const double TSS,
                  const double RSS) {
  ...
  cout << "Model\t" << k << "\t" << (TSS - RSS) << "\t" << (TSS - RSS) / k
       << "\t" << FVal << "\t" << pFVal << endl;
  cout << "Error\t" << nstar - k << "\t" << RSS << "\t" << RSS / (nstar - k)
       << endl;
...
// **************************************************************
void DisplayCoefs(const size_t k, const size_t nstar, const double tstudentval,
                  const double *coefbeta, const double *serbeta) {
...
// **************************************************************
void DisplayStatistics(const size_t n, const size_t nstar, const size_t k,
                       const double RSS, const double R2, const double R2Adj,
                       const double SE) {
...
// **************************************************************
void DisplayCovCorrMatrix(const size_t k, const double sigma, const bool fixed,
                          double **XTWXInv) {
  ...
  for (size_t i = 0; i < k + 1; i++) {
    for (size_t j = 0; j < k + 1; j++) {
      CorrMatrix[i][j] =
          CovMatrix[i][j] / (sqrt(CovMatrix[i][i]) * sqrt(CovMatrix[j][j]));
    }
  }
...
std::vector<double> poly_fit(const std::vector<double> &x_vec,
                             const std::vector<double> &y_vec) {
  const size_t k = x_vec.size() - 1; // Polynomial order
  constexpr bool fixedinter = false; // Fixed the intercept (coefficient A0)
  const int wtype = 0; // Weight: 0 = none (default), 1 = sigma, 2 = 1/sigma^2
  constexpr double fixedinterval =
      0.;                           // The fixed intercept value (if applicable)
  constexpr double alphaval = 0.05; // Critical apha value

  const double *x = x_vec.data();
  auto y_copy = y_vec;
  double *y = y_copy.data();
  std::vector<double> error_vec(x_vec.size(), 0.0);
  const double *erry = error_vec.data();

  // Definition of other variables
  // **************************************************************
  size_t n = x_vec.size(); // Number of data points (adjusted later)
  ...
  if (determinant(Weights, n) == 0.) {
    cout << "One or more points have 0 error. Review the errors on points or "
            "use no weighting. ";
    throw std::logic_error("One or more points have 0 error. Review the errors "
                           "on points or use no weighting.");
  }

  // Calculate the coefficients of the fit
  // **************************************************************
  PolyFit(x, y, n, k, fixedinter, fixedinterval, coefbeta, Weights, XTWXInv);
  std::vector<double> coeffs(coefbeta, coefbeta + k + 1);
  return coeffs;
}
} // namespace xacc