...
void RDMPurificationDecorator::initialize(AcceleratorParameters params) {
  if (!params.count("fermion-observable") &&
      !std::dynamic_pointer_cast<quantum::FermionOperator>(
          params["fermion-observable"].as<std::shared_ptr<Observable>>())) {
    xacc::error(
...
RDMPurificationDecorator::execute(
  ...
  auto rdmGen = xacc::getAlgorithm(
      "rdm", {{"accelerator", decoratedAccelerator}, {"ansatz", ansatz}});
