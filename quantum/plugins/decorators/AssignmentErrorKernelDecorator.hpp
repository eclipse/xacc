...
protected:
  ...
  void generateKernel(std::shared_ptr<AcceleratorBuffer> buffer){
    ...
    int pow_bits = std::pow(2, num_bits);
    AssignmentErrorKernelDecorator::permutations = generatePermutations(num_bits);

    //permutations contains all possible permutationss to generate circuits, there is direct
    //mapping from permutations to circuit as follows:
    //permutations: 10 => X gate on zeroth qubit and nothing on first qubit and measure both
    //permutations: 11 => X gate on both qubits and measure both etc...
