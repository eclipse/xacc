...
TEST(QFastTester, checkSimple) 
{
  ...
  for (int j = 0; j < 5; ++j)
  {
    ...
    const auto runTestCase = [&](bool in_bit0, bool in_bit1, bool in_bit2) {
      ...
      // Check bit string
      const bool result = std::abs(buffer->computeMeasurementProbability(expectedBitString) - 1.0) < 0.1;
      return result;
    };
    ...
    for (int i = 0; i < (1 << 3); ++i)
    {
      allSuccess = allSuccess && runTestCase(i & 0x001, i & 0x002, i & 0x004);
    }
    if (allSuccess)
    {
      std::cout << "Succeeded after " << j << " trials\n";
      break;
    }
    else
    {
