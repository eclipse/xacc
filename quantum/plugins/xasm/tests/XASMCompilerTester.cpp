...
TEST(XASMCompilerTester, checkSimple) {
  ...
  auto compiler = xacc::getCompiler("xasm");
  auto IR = compiler -> compile(R"(__qpu__ void bell(qbit q, double t0, double t1) {
  H(q[0]);
  CX(q[0], q[1]);
  Ry(q[0], t0);
  Ry(q[1], t1);
...
TEST(XASMCompilerTester, checkMultiRegister) {
  ...
  auto compiler = xacc::getCompiler("xasm");
  auto IR = compiler -> compile(R"(__qpu__ void bell_multi(qbit q, qbit r, double t0, double t1) {
  H(q[0]);
  CX(q[0], q[1]);
  H(r[0]);
  CX(r[0], r[1]);
...
//   IR = compiler -> compile(R"(__qpu__ void testFor2(qbit q, std::vector<double> x) {
//   for (int i = 0; i < 5; i++) {
//      H(q[i]);
//      Rx(q[i], x[i]);
//      CX(q[0], q[i]);
