...
TEST(QCMXTester, checkSimple) {
  ...
  qcmx->initialize({{"accelerator",acc}, {"observable", H}, {"ansatz", ansatz},
                    {"cmx-order", 3}, {"expansion-type", "PDS"}});
 
  auto buffer = qalloc(2);
  qcmx->execute(buffer);
  EXPECT_NEAR(-1.14499, buffer->getInformation("opt-val").as<double>(), 1e-3);
}
