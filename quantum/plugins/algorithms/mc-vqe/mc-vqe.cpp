...
bool MC_VQE::initialize(const HeterogeneousMap &parameters) {
  ...
  if (parameters.stringExists("gradient-strategy")){
    gradientStrategy = xacc::getService<AlgorithmGradientStrategy>(parameters.getString("gradient-strategy"));
  ...
  logControl("AIEM Hamiltonian and state preparation parameters [" + std::to_string(prepTime) + " s]", 1);
...
void MC_VQE::execute(const std::shared_ptr<AcceleratorBuffer> buffer) const {
      ...
      [&, this](const std::vector<double> &x, std::vector<double> &dx) {
        ...
        for (int state = 0; state < nStates; state++){ // loop over states
          ...
          if (gradientStrategy){ // gradient-based optimization
            ...
            for (int i = 0; i < nInstructionsEnergy; i++) { // compute energy first
            ...
            // Because AlgorithmGradientStrategy methods take reference to both x and dx
            // we need to keep track of the gradient for each state
          ...
          logControl("State # " + std::to_string(state) + " energy " + std::to_string(energy), 2); 
        }
        ...
        logControl("Optimization iteration finished [" + std::to_string(iterTime) + " s]", 2);
        logControl("Average iteration time [" + std::to_string(iterTime / nStates) + " s]", 2);
  ...
  logControl("Computing Hamiltonian matrix elements in the interference state basis", 1);
  ...
  logControl("Interference basis Hamiltonian matrix elements computed [" + std::to_string(MCTime) + " s]", 1);
  ...
  logControl("MC-VQE simulation finished [" + std::to_string(totalTime) + " s]", 1);
