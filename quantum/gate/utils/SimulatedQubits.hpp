...
public:
  ...
  SimulatedQubits(const std::string &str, int firstIndex, Indices... indices)
      : AcceleratorBuffer(str, firstIndex, indices...),
        bufferState((int)std::pow(2, sizeof...(indices) + 1)) {}

  void applyUnitary(Eigen::MatrixXcd &U) { bufferState = U * bufferState; }

  /**
   * Normalize the state.
   */
  void normalize() { bufferState.normalize(); }
   ...
   */
  QubitState &getState() { return bufferState; }

  /**
   * Set the state.
   * @param st The state to set on this buffer.
   */
  void setState(QubitState &st) { bufferState = st; }
  ...
  virtual const double getExpectationValueZ() {
    if (measurements.empty()) {
      return std::real(
          bufferState.conjugate().transpose().dot(Z * bufferState));
    } else {
  ...
  virtual void print() {
    ...
    for (int i = 0; i < bufferState.size(); i++) {
      ...
      XACCLogger::instance()->info(
          bitStrings[i] + " -> (" + std::to_string(std::real(bufferState(i))) +
