...
public:
  ...
  FunctionalGateInstructionVisitor(HF h, CNF cn, XF x, YF y, ZF z, RXF rx,
      ...
      : hAction(h), cnotAction(cn), xAction(x), yAction(y), zAction(z),
        measureAction(m), condAction(c), rxAction(rx), ryAction(ry),
        rzAction(rz), cpAction(cp), swAction(sw) {}

  void visit(Hadamard &h) { hAction(h); }
  void visit(CNOT &cn) { cnotAction(cn); }
  void visit(X &x) { xAction(x); }

  void visit(Identity &i) {}
  void visit(CZ &i) {}

  void visit(Y &y) { yAction(y); }

  void visit(Z &z) { zAction(z); }
  void visit(Measure &m) { measureAction(m); }
  void visit(ConditionalFunction &c) { condAction(c); }

  void visit(Rx &rx) { rxAction(rx); }
  void visit(Ry &ry) { ryAction(ry); }

  void visit(CPhase &cp) { cpAction(cp); }

  void visit(Rz &rz) { rzAction(rz); }
