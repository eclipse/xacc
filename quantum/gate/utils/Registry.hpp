...
public:
  ...
  bool add(const std::string &id, CreatorFunctionPtr f) {
    if (registry.find(id) != registry.end()) {
      XACCLogger::instance()->info(id +
                                   " already exists in Registry. Ignoring and "
                                   "retaining previous Registry entry");
      return true;
    }
  ...
  bool add(const std::string &id, CreatorFunctionPtr f,
    ...
    bool s2 =
        registryOptions.insert(std::make_pair(id, std::move(options))).second;
    bool s3 = registryOptionHandlers
                  .insert(std::make_pair(id, std::move(optionsHandler)))
                  .second;
    if (!s || !s2) {
      xacc::error("Could not add " + id + " to the Registry.");
      return false;
  ...
  std::vector<std::string> getRegisteredIds() {
    ...
    std::transform(
        ...
        std::bind(&std::map<std::string, CreatorFunctionPtr>::value_type::first,
                  std::placeholders::_1));
    return keys;
  }
  ...
  std::vector<std::shared_ptr<options_description>> getRegisteredOptions() {
    ...
    auto getSecond =
        [](const std::pair<std::string, std::shared_ptr<options_description>>
               &key_val) { return key_val.second; };
    std::transform(registryOptions.begin(), registryOptions.end(),
                   std::back_inserter(values), getSecond);
...
protected:
  ...
  std::map<std::string, std::function<bool(variables_map &)>>
