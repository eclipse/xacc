...
public:
   ...
   */
  static QuantumCircuit getCircuitGraph(const std::string &flatQasmStr) {
    ...
    boost::sregex_token_iterator first{flatQasmStr.begin(), flatQasmStr.end(),
                                       newLineDelim, -1},
        last;
    ...
    for (auto i = boost::sregex_iterator(flatQasmStr.begin(), flatQasmStr.end(),
      ...
      qubitLine.erase(std::remove(qubitLine.begin(), qubitLine.end(), '\n'),
                      qubitLine.end());
      boost::sregex_token_iterator first{qubitLine.begin(), qubitLine.end(),
                                         spaceDelim, -1},
          last;
      std::vector<std::string> splitQubitLine = {first, last};
      qubitVarNameToId[splitQubitLine[1]] = qbitId;
      splitQubitLine[1].erase(std::remove_if(splitQubitLine[1].begin(),
                                             splitQubitLine[1].end(), &isdigit),
    ...
    for (auto line : qasmLines) {
      ...
      if (!boost::contains(line, "qubit") && !boost::contains(line, "cbit")) {
        ...
        boost::sregex_token_iterator first{line.begin(), line.end(), spaceDelim,
                                           -1},
            last;
        ...
        if (incrementLayer(gateCommand, qubitVarNameToId, gateOperations,
                           layer)) {
          layer++;
        }
   ...
   */
  static void
  linkConditionalQasm(QuantumCircuit &mainGraph,
    ...
    int counter = 0;
    int finalIdMainGraph =
        mainGraph.getVertexProperty<2>(mainGraph.order() - 1);
    int finalLayerMainGraph =
        mainGraph.getVertexProperty<1>(mainGraph.order() - 1);
    ...
    for (auto g : conditionalGraphs) {
      ...
      std::get<3>(node.properties) =
          std::vector<int>{conditionalQubits[counter]};
      ...
      for (int i = 0; i < g.order(); i++) {
        ...
        if (i == 0)
          mainGraph.addEdge(id - 1, id);
        if (i == 0)
          initialStateId = id;
        if (i == 0)
          initialStateLayer = std::get<1>(newVert.properties);

        id++;
      }

      generateEdgesFromLayer(initialStateLayer + 1, mainGraph, newGates,
                             initialStateId);
...
private:
  ...
  static void generateEdgesFromLayer(const int layer, QuantumCircuit &graph,
    ...
    int maxLayer =
        std::get<1>(gateOperations[gateOperations.size() - 1].properties);
    ...
    while (currentLayer <= maxLayer) {
      ...
      std::copy_if(gateOperations.begin(), gateOperations.end(),
                   ...
                   [&](const CircuitNode &c) {
                     return std::get<1>(c.properties) == currentLayer;
                   });
  ...
  static bool incrementLayer(const std::vector<std::string> &gateCommand,
                             std::map<std::string, int> &qubitVarNameToId,
                             const std::vector<CircuitNode> &gates,
                             const int &currentLayer) {

    bool oneQubitGate = !boost::contains(gateCommand[1], ","),
         noGateAtQOnL = true;
    auto g = boost::to_lower_copy(gateCommand[0]);
    boost::trim(g);
    if (g == "measz")
    ...
    std::copy_if(gates.begin(), gates.end(), std::back_inserter(thisLayerGates),
                 [&](const CircuitNode &c) {
                   return std::get<1>(c.properties) == currentLayer;
                 });
    ...
    if (!oneQubitGate) {
      ...
      return true;
               ...
               (std::get<0>(gates[gates.size() - 1].properties) == "measure") &&
               g != "measure") {
      return true;
    }
