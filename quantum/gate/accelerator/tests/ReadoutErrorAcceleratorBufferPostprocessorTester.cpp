...
public:
  ...
  virtual const double getExpectationValueZ() { return .88; }
};

class FakeMeasureAB : public AcceleratorBuffer {
public:
  FakeMeasureAB(const std::string &str, const int N)
...
TEST(ReadoutErrorAcceleratorBufferPostprocessorTester, checkSimple) {
  ...
  auto f01 =
      std::make_shared<GateFunction>("measure0_qubit_0", "readout-error");
  ...
  auto f10 =
      std::make_shared<GateFunction>("measure1_qubit_0", "readout-error");
  ...
  ReadoutErrorAcceleratorBufferPostprocessor processor(*ir, sites,
                                                       orderedTerms);
  ...
  xacc::Finalize();
  //	EXPECT_TRUE(std::fabs(fixed[0]->getExpectationValueZ() - .911111) <
  // 1e-6);
}
