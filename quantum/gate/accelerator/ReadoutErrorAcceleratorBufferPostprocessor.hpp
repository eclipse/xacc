...
public:
  ...
  virtual const double getExpectationValueZ() { return expectationValue; }
};

class ReadoutErrorAcceleratorBufferPostprocessor
    : public AcceleratorBufferPostprocessor {
...
protected:
  ...
  fix_assignments(std::map<std::string, double> oldExpects,
                  std::map<std::string, std::vector<int>> sites,
                  std::map<int, std::pair<double, double>> errorRates);

public:
  ReadoutErrorAcceleratorBufferPostprocessor(
      IR &i, std::map<std::string, std::vector<int>> sitesMap,
  ...
  process(std::vector<std::shared_ptr<AcceleratorBuffer>> buffers);
