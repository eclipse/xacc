...
ReadoutErrorAcceleratorBufferPostprocessor::process(
  ...
  if (buffers.size() % nonIdentityKernels != 0) {
    xacc::error(
        "ReadoutError Postprocessor: Invalid number of buffers and kernels - " +
        std::to_string(buffers.size()) + ", " +
        std::to_string(nonIdentityKernels));
  }

  std::vector<std::vector<std::shared_ptr<AcceleratorBuffer>>> bufvec;
  for (int i = 0; i < buffers.size(); i += nonIdentityKernels) {
    std::vector<std::shared_ptr<AcceleratorBuffer>> sub(
        buffers.begin() + i, buffers.begin() + i + nonIdentityKernels);
  ...
  for (auto subList : bufvec) {
    ...
    for (int i = allTerms.size(); i < nIk; i++) {
      ...
      if (counter == 2) {
        errorRates.insert({qbitCount,
                           {std::isnan(probs[0]) ? 0.0 : probs[0],
    ...
    for (auto &kv : errorRates) {
      ...
      s << "Qubit " << kv.first << ": p01 = " << kv.second.first
        << ", p10 = " << kv.second.second;
      xacc::info(s.str());
    }

    // Return new AcceleratorBuffers subtype, StaticExpValAcceleratorBuffer that
    // has static

    std::map<std::string, double> oldExpects;
    for (int i = 0; i < allTerms.size(); i++) {
      xacc::info("Raw Expectatations: " + allTerms[i] + " = " +
                 std::to_string(subList[i]->getExpectationValueZ()));
    ...
    for (int i = 0; i < allTerms.size(); i++) {
      auto staticBuffer = std::make_shared<StaticExpectationValueBuffer>(
          subList[i]->name(), subList[i]->size(), fixed[allTerms[i]]);
...
ReadoutErrorAcceleratorBufferPostprocessor::fix_assignments(
    std::map<std::string, double> oldExpects,
  ...
  for (auto &kv : sites) {
    ...
    if (kv.first != "I") {
      if (kv.second.size() == 1) {
        ...
        std::stringstream s2;
        newExpects.insert(
            {kv.first, exptZZ(oldExpects[kv.first], oldExpects[k0],
                              oldExpects[k1], ap10, ap01, bp10, bp01)});

      } else {
        xacc::error(
            "Correction for paulis with support on > 2 sites not implemented.");
      }
    }
  }
...
double ReadoutErrorAcceleratorBufferPostprocessor::exptZZ(
    double E_ZZ, double E_ZI, double E_IZ, double a01, double a10, double b01,
    double b10, bool averaged) {
  ...
  double E_ZZ_fixed = 0;
  if (averaged) {
    E_ZZ_fixed = (E_ZZ + (a01 - a10) * (-E_ZI - E_IZ + a01 - a10)) /
                 std::pow((-1 + a01 + a10), 2);
  } else {
    E_ZZ_fixed =
        (-(2 - a00 * b01 - a11 * b01 - a00 * b10 - a11 * b10 +
           a01 * a01 * (b00 * b01 + b10 * (2 * b01 + b11)) +
           a10 * a10 * (b00 * b01 + b10 * (2 * b01 + b11)) +
           a01 * ((a00 + 2 * a10) * b01 * b01 - 2 * b10 + a00 * b10 * b10 +
                  2 * a10 * b10 * b10 +
                  b00 * (-1 + a11 * b01 + a00 * b10 + 2 * a10 * b10) - b11 +
                  a11 * b10 * b11 +
                  b01 * (-2 + 2 * a11 * b10 + a00 * b11 + 2 * a10 * b11)) +
           a10 * (a11 * b01 * b01 - 2 * b10 + a11 * b10 * b10 +
                  b00 * (-1 + a00 * b01 + a11 * b10) - b11 + a00 * b10 * b11 +
                  b01 * (-2 + 2 * a00 * b10 + a11 * b11))) *
             E_ZZ -
         a10 * b00 * E_ZI + a00 * b01 * E_ZI + a11 * b01 * E_ZI +
         a00 * a10 * b00 * b01 * E_ZI + a10 * a10 * b00 * b01 * E_ZI -
         2 * a00 * a11 * b01 * b01 * E_ZI - a10 * a11 * b01 * b01 * E_ZI -
         a00 * b10 * E_ZI - a11 * b10 * E_ZI + a10 * a11 * b00 * b10 * E_ZI +
         2 * a00 * a11 * b10 * b10 * E_ZI + a10 * a11 * b10 * b10 * E_ZI +
         a10 * b11 * E_ZI - a10 * a11 * b01 * b11 * E_ZI -
         a00 * a10 * b10 * b11 * E_ZI - a10 * a10 * b10 * b11 * E_ZI -
         a10 * b00 * E_IZ - a00 * b01 * E_IZ + a11 * b01 * E_IZ +
         a00 * a10 * b00 * b01 * E_IZ + a10 * a10 * b00 * b01 * E_IZ -
         a10 * a11 * b01 * b01 * E_IZ - a00 * b10 * E_IZ + a11 * b10 * E_IZ -
         a10 * a11 * b00 * b10 * E_IZ + 2 * a00 * a10 * b01 * b10 * E_IZ -
         a10 * a11 * b10 * b10 * E_IZ - a10 * b11 * E_IZ +
         2 * a10 * a10 * b00 * b11 * E_IZ - a10 * a11 * b01 * b11 * E_IZ +
         a00 * a10 * b10 * b11 * E_IZ + a10 * a10 * b10 * b11 * E_IZ -
         a10 * b00 * E_II + a00 * b01 * E_II - a11 * b01 * E_II +
         a00 * a10 * b00 * b01 * E_II + a10 * a10 * b00 * b01 * E_II +
         2 * a10 * a11 * b00 * b01 * E_II + a10 * a11 * b01 * b01 * E_II -
         2 * a00 * a10 * a11 * b00 * b01 * b01 * E_II -
         2 * a10 * a10 * a11 * b00 * b01 * b01 * E_II - a00 * b10 * E_II +
         a11 * b10 * E_II - a10 * a11 * b00 * b10 * E_II +
         ...
         2 * a10 * a10 * a11 * b01 * b10 * b10 * E_II + a10 * b11 * E_II +
         a10 * a11 * b01 * b11 * E_II -
         2 * a10 * a10 * a11 * b00 * b01 * b11 * E_II -
         a00 * a10 * b10 * b11 * E_II - a10 * a10 * b10 * b11 * E_II -
         ...
         a01 * a01 *
             (-b10 * (2 * a00 * b01 * (-b01 + b10) * E_II +
                      b11 * (E_ZI + E_IZ +
                             (-1 - 2 * a00 * b01 - 2 * a10 * b01 +
                              2 * a00 * b10 + 2 * a10 * b10) *
                                 E_II)) +
              b00 * (2 * (a00 + a10) * b01 * b01 * E_II -
                     2 * b11 * (E_IZ + (a00 + 2 * a10) * b10 * E_II) +
                     b01 * (E_ZI - E_IZ +
                            (-1 - 2 * a00 * b10 - 2 * a10 * b10 +
                             2 * a00 * b11 + 4 * a10 * b11) *
                                E_II))) +
         a01 *
             (b11 * E_ZI - 2 * a10 * b01 * b11 * E_ZI - a11 * b10 * b11 * E_ZI -
              2 * a11 * b01 * b10 * E_IZ + b11 * E_IZ - a11 * b10 * b11 * E_IZ +
              2 * a10 * a11 * b01 * b01 * b10 * E_II -
              2 * a10 * a11 * b01 * b10 * b10 * E_II - b11 * E_II +
              a11 * b10 * b11 * E_II - 2 * a10 * a10 * b01 * b10 * b11 * E_II +
              2 * a10 * a11 * b01 * b10 * b11 * E_II +
              2 * a10 * a10 * b10 * b10 * b11 * E_II -
              2 * a10 * a11 * b10 * b10 * b11 * E_II +
              b00 *
                  ((-1 + a11 * b01 + a00 * b10 + 2 * a10 * b10) * E_ZI +
                   (1 - a11 * b01 + a00 * b10) * E_IZ +
                   (1 - 2 * a10 * a10 * b01 * b01 + 2 * a10 * a10 * b01 * b10 -
                    4 * a10 * a10 * b01 * b11 + 4 * a10 * a10 * b10 * b11 +
                    a11 * (2 * a10 * b01 * b01 - 2 * a10 * b10 * b11 +
                           b01 * (-1 - 2 * a10 * b10 + 2 * a10 * b11)) +
                    a00 * (-2 * (a10 - a11) * b01 * b01 + b10 +
                           2 * a10 * b10 * b11 -
                           2 * b01 * (1 + a11 * b10 + a10 * (-b10 + b11)))) *
                       E_II) +
              a00 * (-b01 * b01 *
                         (E_ZI - E_IZ + E_II + 2 * a10 * b10 * E_II -
                          4 * a11 * b10 * E_II) -
        ...
        (2 *
         ((-1 + a11 * (b01 + b10)) *
              (1 - a00 * (b01 + b10) + a10 * a10 * (b00 + b10) * (b01 + b11) +
               a10 * (-b01 + b00 * (-1 + a00 * b01) - b10 +
                      2 * a00 * b01 * b10 - b11 + a00 * b10 * b11)) +
          a01 * a01 *
              (b10 * (a00 * b01 * b01 + (-1 + a00 * b10 + a10 * b10) * b11 +
                      b01 * (-1 + a10 * b11 + a00 * (b10 + b11))) +
               b00 * ((a00 + a10) * b01 * b01 +
                      (-1 + a00 * b10 + 2 * a10 * b10) * b11 +
                      b01 * (-1 + a10 * b10 + 2 * a10 * b11 +
                             a00 * (b10 + b11)))) +
          a01 *
              (b01 * b01 *
                   (a10 * (-1 + a11 * b10) +
                    a00 * (-1 + a10 * b10 + 2 * a11 * b10)) +
               (-1 + a00 * b10 + a10 * b10) *
                   (-b11 + b10 * (-1 + a10 * b11 + a11 * b11)) +
               b01 * (1 - 2 * a10 * b11 + a10 * a10 * b10 * b11 +
                      a11 * b10 * (-2 + a10 * (b10 + b11)) +
                      a00 * ((a10 + 2 * a11) * b10 * b10 - b11 +
                             b10 * (-2 + a10 * b11 + a11 * b11))) +
               b00 *
                   (1 - a11 * b01 + a10 * a10 * (b01 + b10) * (b01 + 2 * b11) +
                    a00 * (b01 + b10) * (-1 + a11 * b01 + a10 * (b01 + b11)) +
                    a10 *
                        (a11 * b01 * b01 - 2 * b10 - 2 * b11 + a11 * b10 * b11 +
                         b01 * (-2 + a11 * (b10 + b11)))))));
  }
  return E_ZZ_fixed;
}
