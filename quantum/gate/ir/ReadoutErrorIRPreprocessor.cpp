...
namespace xacc {
namespace quantum {
std::shared_ptr<AcceleratorBufferPostprocessor>
ReadoutErrorIRPreprocessor::process(IR &ir) {
  ...
  if (!nullBufferPostprocessor &&
      xacc::optionExists("no-readout-error-postprocess")) {
    nullBufferPostprocessor = true;
  }
  ...
  extraKernelsNeeded.erase(
      std::unique(extraKernelsNeeded.begin(), extraKernelsNeeded.end()),
      extraKernelsNeeded.end());
  ...
  for (auto o : extraKernelsNeeded) {
    ...
    auto meas =
        gateRegistry->createInstruction("Measure", std::vector<int>{bit});
  ...
  for (int i = 0; i < 2 * nQubits; i += 2) {
    ...
    auto meas01 = gateRegistry->createInstruction(
        "Measure", std::vector<int>{qubitMap[qbit]});
    ...
    auto x =
        gateRegistry->createInstruction("X", std::vector<int>{qubitMap[qbit]});
    auto meas10 = gateRegistry->createInstruction(
        "Measure", std::vector<int>{qubitMap[qbit]});
  ...
  if (nullBufferPostprocessor) {
    ...
    return std::make_shared<ReadoutErrorAcceleratorBufferPostprocessor>(
        ir, sites, orderedPauliTerms);
  }
}
