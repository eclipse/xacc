...
const std::string KernelReplacementPreprocessor::process(
    const std::string &source, std::shared_ptr<Compiler> compiler,
  ...
  for (auto position : allPositions) {
    ...
    auto funcString =
        source.substr(position, source.find("\n", position) - position);
    ...
    if (boost::contains(split[2], ",")) {
      bufName =
          split[2].substr(parenLocation, split[2].find(",") - parenLocation);
    } else {
      bufName =
          split[2].substr(parenLocation, split[2].find(")") - parenLocation);
    }
