...
TEST(DirectedGraphTester, checkConstruction) {
  ...
  DirectedBoostGraph g1;
  g1.addVertex();
  EXPECT_TRUE(g1.order() == 1);

  // Create with 5 vertices
  DirectedBoostGraph g2(5);
  EXPECT_TRUE(g2.order() == 5);

  // Create a 3 node graph
  DirectedBoostGraph graph;
  graph.addVertex({{"bias",0.0}});
  graph.addVertex({{"bias",0.0}});
  graph.addVertex({{"bias",0.0}});
  ...
  EXPECT_EQ(0.0 , graph.getVertexProperty(0, "bias").as<double>());
  EXPECT_EQ(0.0 , graph.getVertexProperty(1, "bias").as<double>());
  graph.setVertexProperty(0, "bias", 3.3);
  EXPECT_EQ(3.3 , graph.getVertexProperty(0, "bias").as<double>());
...
TEST(DirectedGraphTester, checkEdgeExists) {
...
TEST(DirectedGraphTester, checkDegree) {
  DirectedBoostGraph complete5(5);
  ...
  int start = 4;
  for (int i = 0; i < 5; i++) {
    EXPECT_EQ(start , complete5.degree(i));
    start--;
  }
}

// TEST(DirectedGraphTester, checkDiameter) {
...
// TEST(DirectedGraphTester, checkWrite) {
