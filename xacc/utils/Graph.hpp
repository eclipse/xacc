...
template <typename T>
struct is_valid_vertex<T, decltype(std::declval<T>().properties, void())>
    : std::true_type {};

template <typename... Properties> class XACCVertex {
public:
  std::tuple<Properties...> properties;
  std::vector<std::string> propertyNames;
  XACCVertex() : propertyNames(sizeof...(Properties)) {}
  std::string getPropertyName(const int index) { return propertyNames[index]; }
};
 ...
 * s
 */
template <typename Vertex, GraphType type = Undirected> class Graph {

  // Make sure we've been given a valid Vertex
  static_assert(is_valid_vertex<Vertex>::value,
                "XACC Graph can only take Vertices that are derived from "
                "XACCVertex, or have a tuple properties member.");

  using graph_type = typename std::conditional<(type == GraphType::Undirected),
                                               undirectedS, directedS>::type;

  // Setup some easy to use aliases
  using adj_list =
      adjacency_list<vecS, vecS, graph_type, Vertex, EdgeWeightProperty>;
  using BoostGraph = std::shared_ptr<adj_list>;
  using vertex_type = typename boost::graph_traits<adjacency_list<
      vecS, vecS, graph_type, Vertex, EdgeWeightProperty>>::vertex_descriptor;
  using edge_type = typename boost::graph_traits<adjacency_list<
      vecS, vecS, graph_type, Vertex, EdgeWeightProperty>>::edge_descriptor;
...
protected:
  ...
  public:
    XACCVertexPropertiesWriter(adj_list &list) : graph(list) {}
...
public:
   ...
   */
  Graph() { _graph = std::make_shared<adj_list>(); }
  ...
  Graph(const int numberOfVertices) {
    _graph = std::make_shared<adj_list>(numberOfVertices);
  }
  ...
  // typename std::enable_if<type == Directed>::type getTargetVertices(const int
  // vertexIndex, std::vector<int>& connectedVerts) {
  //     typename graph_traits < BoostGraph >::out_edge_iterator ei, ei_end;
  //     auto u = getVertex(vertexIndex);
  //     for (boost::tie(ei, ei_end) = out_edges(u, *_graph.get()); ei !=
  //     ei_end; ++ei) {
  ...
  //         // std::cout << "There is an edge from " << source <<  " to " <<
  //         target << std::endl;
  //     }
  // }
   ...
   */
  void addVertex() { add_vertex(*_graph.get()); }
   ...
   * @param properties
   */
  template <typename... Properties> void addVertex(Properties... properties) {
  ...
  template <const int PropertyIndex,
            typename DeducedPropertyType> // const int PropertyIndex>
  void setVertexProperty(const int index, DeducedPropertyType prop) {
    //			decltype(std::get<PropertyIndex>(std::declval<Vertex>().properties))
    // prop) {
    auto v = vertex(index, *_graph.get());
  ...
  auto getVertexProperties(const int index)
      -> decltype((*_graph.get())[index].properties) {
  ...
  template <const int PropertyIndex>
  decltype(std::get<PropertyIndex>(std::declval<Vertex>().properties)) &
  getVertexProperty(const int index) {
    auto v = vertex(index, *_graph.get());
   ...
   */
  int size() { return num_edges(*_graph.get()); }
   ...
   */
  int order() { return num_vertices(*_graph.get()); }

  std::list<int> getNeighborList(const int index) {
    std::list<int> l;

    typedef typename boost::property_map<adj_list, boost::vertex_index_t>::type
        IndexMap;
    IndexMap indexMap = get(boost::vertex_index, *_graph.get());

    typedef typename boost::graph_traits<adj_list>::adjacency_iterator
        adjacency_iterator;
  ...
  void computeShortestPath(int startIndex, std::vector<double> &distances,
    ...
    typename property_map<adj_list, edge_weight_t>::type weightmap =
  ...
  const int depth() {
    if (type == Undirected) {
      XACCLogger::instance()->error(
          "Cannot compute depth on undirected graph.");
    }
