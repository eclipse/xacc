...
public:
  ...
  createBuffer(const std::string &varId) = 0;
   ...
   */
  virtual std::shared_ptr<AcceleratorBuffer>
  createBuffer(const std::string &varId, const int size) = 0;
  ...
  virtual const double getExecutionTime() {
    XACCLogger::instance()->info("getExecutionTime() not implemented by this "
                                 "Accelerator, returning 0.0.");
    return 0.0;
  }
