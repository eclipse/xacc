...
struct MSpace : public spp_rc {
    MSpace() : _sp(create_mspace(0, 0)) {}

    ~MSpace() { destroy_mspace(_sp); }
...
class spp_allocator {
  public:
    ...
    void swap(spp_allocator& o) { std::swap(_space, o._space); }

    pointer allocate(size_t n, const_pointer /* unused */ = 0) {
        pointer res =
            static_cast<pointer>(mspace_malloc(_space->_sp, n * sizeof(T)));
        if (!res)
            throw std::bad_alloc();
        return res;
    }
    ...
    pointer reallocate(pointer p, size_t new_size) {
        pointer res = static_cast<pointer>(
            mspace_realloc(_space->_sp, p, new_size * sizeof(T)));
        if (!res)
            throw std::bad_alloc();
        return res;
    }
    ...
    template <class U>
    struct rebind {
        // rebind to libc_allocator because we want to use malloc_inspect_all in
        // destructive_iterator to reduce peak memory usage (we don't want
        // <group_items> mixed with value_type when we traverse the allocated
        // memory).
    ...
    // check if we can clear the whole allocator memory at once => works only if
    // the allocator is not be shared. If can_clear() returns true, we expect
    // that the next allocator call will be clear() - not allocate() or
    // deallocate()
    bool can_clear() {
...
// allocators are "equal" whenever memory allocated with one can be deallocated
// with the other
template <class T>
inline bool operator==(const spp_::spp_allocator<T>& a,
                       const spp_::spp_allocator<T>& b) {
...
static size_t align_offset(void* p) {
    return (((size_t) p & spp_chunk_align_mask) == 0)
               ? 0
               : ((SPP_MALLOC_ALIGNMENT - ((size_t) p & spp_chunk_align_mask)) &
                  spp_chunk_align_mask);
}
...
#define SPP_DEFAULT_TRIM_THRESHOLD                                             \
...
#if SPP_HAVE_MREMAP
#ifndef WIN32
#define SPP_MREMAP_DEFAULT(addr, osz, nsz, mv)                                 \
...
#else
#define SPP_CALL_MREMAP(addr, osz, nsz, mv)                                    \
    SPP_MREMAP_DEFAULT((addr), (osz), (nsz), (mv))
#endif
#else
...
static const unsigned MIN_CHUNK_SIZE =
    (MCHUNK_SIZE + spp_chunk_align_mask) & ~spp_chunk_align_mask;
...
static SPP_FORCEINLINE size_t pad_request(size_t req) {
    return (req + CHUNK_OVERHEAD + spp_chunk_align_mask) &
           ~spp_chunk_align_mask;
}
...
class malloc_state {
  public:
    ...
    /* ------------------------ Relays to internal calls to malloc/free from
     * realloc, memalign etc */
    ...
    SPP_IMPL void** ialloc(size_t n_elements, size_t* sizes, int opts,
                           void* chunks[]);
  ...
  private:
    ...
    /* ------------------------ Addressing by index. See  about smallbin
     * repositioning --- */
    ...
    SPP_FORCEINLINE void unlink_first_small_chunk(mchunkptr B, mchunkptr P,
                                                  bindex_t I);
    ...
    static const size_t MAX_SMALL_REQUEST =
        (MAX_SMALL_SIZE - spp_chunk_align_mask - CHUNK_OVERHEAD);
...
static malloc_state* get_mstate_for(malloc_chunk_header* p) {
    return (malloc_state*) (((mchunkptr)((char*) (p) + (p->chunksize())))
                                ->prev_foot ^
                            mparams._magic);
}
#endif
...
int malloc_params::_init() {
    ...
    if (_magic == 0) {
        ...
        gsize =
            ((SPP_DEFAULT_GRANULARITY != 0) ? SPP_DEFAULT_GRANULARITY : psize);
#else
        {
...
void malloc_state::do_check_inuse_chunk(mchunkptr p) const {
    ...
    assert(p->is_mmapped() || p->pinuse() ||
           (mchunkptr) p->prev_chunk()->next_chunk() == p);
    if (p->is_mmapped())
...
void malloc_state::do_check_tree(tchunkptr t) {
    ...
    do {
        ...
        if (u->_parent == 0) {
            ...
            head = u;
            assert(u->_parent != u);
            assert(u->_parent->_child[0] == u || u->_parent->_child[1] == u ||
                   *((tbinptr*) (u->_parent)) == u);
            if (u->_child[0] != 0) {
...
size_t malloc_state::traverse_and_check() {
    ...
    if (is_initialized()) {
        ...
        while (s != 0) {
            ...
            mchunkptr lastq = 0;
            assert(q->pinuse());
            while (segment_holds(s, q) && q != _top &&
                   q->_head != FENCEPOST_HEAD) {
                sum += q->chunksize();
                if (q->is_inuse()) {
                    assert(!bin_find(q));
                    do_check_inuse_chunk(q);
                    ...
                    assert(lastq == 0 ||
                           lastq->is_inuse()); // Not 2 consecutive free
...
void malloc_state::unlink_first_small_chunk(mchunkptr B, mchunkptr p,
                                            bindex_t I) {
    mchunkptr F = p->_fd;
    assert(p != B);
    assert(p != F);
    ...
    if (!treemap_is_marked(I)) {
        ...
        *H = X;
        ...
        for (;;) {
            if (T->chunksize() != s) {
                tchunkptr* C =
                    &(T->_child[(K >> (spp_size_t_bitsize - 1)) & 1]);
                K <<= 1;
                if (*C != 0)
                    T = *C;
...
mchunkptr malloc_state::mmap_resize(mchunkptr oldp, size_t nb, int flags) {
    ...
    if (oldsize >= nb + sizeof(size_t) &&
        ...
        size_t newmmsize =
            mmap_align(nb + 6 * sizeof(size_t) + spp_chunk_align_mask);
        char* cp = (char*) SPP_CALL_MREMAP((char*) oldp - offset, oldmmsize,
                                           newmmsize, flags);
        if (cp != cmfail) {
...
void* malloc_state::sys_alloc(size_t nb) {
    ...
    /*
      Try getting memory with a call to MMAP new space (disabled if not
      SPP_HAVE_MMAP). We need to request enough bytes from system to ensure we
      can malloc nb bytes upon success, so pad with enough space for top_foot,
      plus alignment-pad to make sure we don't lose bytes if not on boundary,
      and round this up to a granularity unit.
    */
...
size_t malloc_state::release_unused_segments() {
    ...
    _release_checks = (((size_t) nsegs > (size_t) SPP_MAX_RELEASE_CHECK_RATE)
                           ? (size_t) nsegs
                           : (size_t) SPP_MAX_RELEASE_CHECK_RATE);
    return released;
}
...
int malloc_state::sys_trim(size_t pad) {
    ...
    if (pad < MAX_REQUEST && is_initialized()) {
        ...
        if (_topsize > pad) {
            // Shrink top space in _granularity - size units, keeping at least
            // one
            size_t unit = mparams._granularity;
            size_t extra = ((_topsize - pad + (unit - 1)) / unit - 1) * unit;
            msegmentptr sp = segment_holding((char*) _top);

            if (!sp->is_extern_segment()) {
                if (sp->is_mmapped_segment()) {
                    if (SPP_HAVE_MMAP && sp->_size >= extra &&
                        !has_segment_link(sp)) {
                        ...
                        if ((SPP_CALL_MREMAP(sp->_base, sp->_size, newsize,
                                             0) != mfail) ||
...
void* malloc_state::_malloc(size_t bytes) {
    if (1) {
        ...
        if (bytes <= MAX_SMALL_REQUEST) {
            ...
            if ((smallbits & 0x3U) != 0) {
                ...
                if (smallbits != 0) {
                    ...
                    binmap_t leftbits = (smallbits << idx) &
                                        left_bits(malloc_state::idx2bit(idx));
                    binmap_t leastbit = least_bit(leftbits);
                    bindex_t i = compute_bit2idx(leastbit);
                    b = smallbin_at(i);
                    p = b->_fd;
            ...
            nb = spp_max_size_t; // Too big to allocate. Force failure (in sys
                                 // alloc)
        else {
        ...
        if (rtcheck(ok_address(p) && ok_inuse(p))) {
            ...
            if (!p->pinuse()) {
                ...
                if (p->is_mmapped()) {
                    ...
                    mchunkptr prev =
                        (mchunkptr) p->chunk_minus_offset(prevsize);
                    psize += prevsize;
                    p = prev;
                    if (rtcheck(ok_address(prev))) {
                        // consolidate backward
...
mchunkptr malloc_state::try_realloc_chunk(mchunkptr p, size_t nb,
                                          int can_move) {
    mchunkptr newp = 0;
    size_t oldsize = p->chunksize();
    mchunkptr next = (mchunkptr) p->chunk_plus_offset(oldsize);
    if (rtcheck(ok_address(p) && ok_inuse(p) && ok_next(p, next) &&
                ok_pinuse(next))) {
        if (p->is_mmapped())
            ...
            if ((((size_t)(mem)) & (alignment - 1)) != 0) {
                ...
                */
                char* br = (char*) mem2chunk(
                    (void*) (((size_t)((char*) mem + alignment - 1)) &
                             -alignment));
