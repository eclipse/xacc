...
public:
	...
	PyXACCVisitor(std::vector<InstructionParameter> functionParams) {
        pyxaccStr += "@qpu()\ndef foo(" + boost::lexical_cast<std::string>(functionParams[0]);
	...
	void visit(CZ& cz) {
		pyxaccStr += "   CZ(" + std::to_string(cz.bits()[0]) + ", " + std::to_string(cz.bits()[1]) + ")\n";
	}
	 ...
	 */
	void visit(CNOT& cn) {
		pyxaccStr += "   CNOT(" + std::to_string(cn.bits()[0]) + ", " + std::to_string(cn.bits()[1]) + ")\n";
	}
	/**
	 * Visit X gates
	 */
	...
	void visit(Swap& s) {
		pyxaccStr += "   SWAP(" + std::to_string(s.bits()[0]) + ", " + std::to_string(s.bits()[1]) + ")\n";
	}
